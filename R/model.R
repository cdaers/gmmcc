    ...
    for (iter in 1:chain_length) {
      ...
      if (proc_name == "test") {
        ...
        if (!is.null(model_gmm[[chain_num]][[iter]])) {
          pred_temp <- predict(model_gmm[[chain_num]][[iter]], x[[x_nm]])
      ...
      if (draw_de) {
        png(sprintf("%s/%s_%s_%s_%s.png", graph_dir_gmm, x_nm, proc_name, iter, chain_num))
        plot(mclust::densityMclust(x[[x_nm]]), main = sprintf("MBDE on %s in %s set", x_nm, proc_name),
             ylab = "MBDE", xlab = x_nm, what = "density")
        points(x[[x_nm]], rep(0,length(x[[x_nm]])), pch = 19, col = y + 2)
        dev.off()
      }
...
model_train <- function(x, y,
  ...
  if ("lr" %in% model_names) {
    ...
    if (length(x_selected) > 0) {
      ...
      y_pred <- predict(model_lr, as.matrix(x_lr), type = "class")
      if (family == "binomial") {
        y_prob <- as.numeric(predict(model_lr, as.matrix(x_lr), type = "probs"))
      } else {
        y_prob <- as.numeric(apply(predict(model_lr, as.matrix(x_lr), type = "probs"), 1, max))
      }
...
#' @param strafity If data is split in a stratified fashion, default TRUE.
#'
#' @param model_names Model names, default linear regression (lr) and GMM classifier (gmm).
#' @param thres Accuracy threshold, default 0.98.
#' @param min_thres accuracy rate needed, default 0.92.
...
model_run <- function(dat, dat_nm = "data", # data
                      ...
                      rep_labels = 1L:10L, seed = 2025, model_saved = FALSE,
  ...
  print(template_rec)
  write.csv(metric_rec, sprintf("%s/%s_metric_rec.csv", result_dir, dat_nm), row.names= F)
  write.csv(template_rec, sprintf("%s/%s_template_rec.csv", result_dir, dat_nm), row.names= F)
...
model_stab_test <- function(dat, dat_nm = "data", target_var = "target",
  ...
  write.csv(stab_test_result, sprintf("%s/%s_model_stab_test.csv", result_dir, dat_nm), row.names= F)
...
stab_test_run <- function(dat, dat_nm,
  ...
  result <- model_stab_test(dat, dat_nm, model = model_chosen, no_chain = model_ref$chain_num)
